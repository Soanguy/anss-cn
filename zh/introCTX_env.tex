...
\usemodule[memos]
  [ bodyfont=sourcehans,
    fallbackfont=pagella,
    themecolor=purple,
    layout=exotic,
    textcount=35,
    margincount=10,
    chapterstyle=kaolike,
    hdrstyle=kaolike,
    tocstyle=kaolike,
  ]
\usemodule[zhpunc]
...
\startenvironment introCTX_env.tex
    \setuplayout[hermes][backspace=20mm,rightmargindistance=5mm]
    
    \definebodyfontenvironment [10.5bp]
    \setupbodyfontenvironment  [10.5bp][
      text=10.5bp,script=9.00bp,scriptscript=7.50bp,
        a=14.0bp,     b=16.0bp,      c=22.0bp,
        d=26.0bp,     x=8.80bp,     xx=6.60bp,
      big=14.0bp, small=9.00bp,        em=italicface]

    \setupbodyfont[10.5bp]
    \setupfontexpansion[conservative]
                       [stretch=1, shrink=1, step=0.25, factor=1]
    
    \definefontfeature[default][default][expansion=conservative]
...
%    \setupitemize[indenting=none] % First line of paragraphs not indented
    \setupitemgroup [itemize] [1] [symbol=mp:bullet]
    \setupitemgroup [itemize] [2] [symbol=diamond]
    \setupitemgroup [itemize] [3] [symbol=asterisk]
    \setupitemgroup [itemize] [4] [symbol=star]
...
%    \setupuserpagenumber[number=2] i dont know why, but it conflict with memos when doublesided is enabled?
...
%    \setuphead[part][number=no]
    ...
    \def\stopDoubleExample{\stopcolumns\egroup\par}

    % Small print
    \definestartstop[SmallPrint]
                    [
                        before={\startnarrower\hw\switchtobodyfont[small]},
                        after={\stopnarrower},
                    ]
    \setuptyping[before=\setupwhitespace[none]\setuplocalinterlinespace[line=12pt]\blank,]
    ...
    \overloaded\tolerant\def\ShowAndExecute [#1]#*[#2]
    {
    ...
    \defineparagraphs[ShowAndTypeset]
                     [n=2, rule=on, rulecolor=darkmagenta,
                      distance=0.75cm, tolerance=verytolerant]
    \unprotect % Allow _ and @ in macro names
    \overloaded\tolerant\def\ShowAndTypeset[#1]#*[#2]#:#*#={\begingroup%
    ...
    \define\Conjecture
        {\inoutermargin[line=1]{\externalfigure[miniconjecture.jpg][width=2em]}}

    % Doubt: Print the doubt icon in the margin
    \define\Doubt
        {\inoutermargin[line=1]{\externalfigure[minidoubt.jpg][width=2em]}}
    ...
    \define[1]\MyKey{\penalty0\quotation{\tt #1}}
    ...
    \setuphead[chapter][after={\vskip2\baselineskip\relax
          \outermargin{\noindent
           {\bf Table of Contents:}\par
           \setuplist
               [section]
               [pagenumber=no, width=fit,margin=0mm,align=inner, style=\itx]%
          \noindent\placecontent[list=section]}}]
          
    \setuphead[title][after={\vskip2\baselineskip\relax}]
...
%    \setupalign[hz, lesshyphenated, verytolerant, stretch]
    ...
    \def\SNOTE#1{\start
      \writestatus{SNOTE}{这里是新增的内容，需要进行甄别。}%
      \inoutermargin{\global\advance\SNOTECOUNT by \plusone%
      \startDualHeaderText[showtitle=0,number={\bf\symbol[fontawesome][info_sign] \high{\the\SNOTECOUNT}},width=\hsize,headerbgcolor=lightthemecolor]
      \noindent  #1
      \stopDualHeaderText
      }
    \stop{\color[themecolor]{\symbol[fontawesome][info_sign]\high{\the\SNOTECOUNT}\ }}}

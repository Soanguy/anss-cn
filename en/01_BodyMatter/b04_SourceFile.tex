...
%%%              taken from Chapter 2 of The TeX Book: "Book printing
%%%              versus ordinary typing".  Basically, it is about
%%%              explaining how to write the source file.  Initially, in
%%%              this chapter I included part of what finally became
%%%              Chapter 10 and appendix B.  But at the last miniute I
%%%              decided to reconfigure it and link it to dealing with
%%%              projects that I initially planned to deal with at the
%%%              end.  I think that this final order is preferable.
%%%
%%% Edited with: Emacs + AUCTeX - And at times with vim + context-plugin
%%%

% TODO: there is a lot of material in here that isn't required for beginner
% ConTeXt users (IMHO).  Specifically, knowing about projects, products and
% components is a lot of info for someone who wants to write a 5 or 10 page
% document.  I think that knowing about "environments" and /input is good
% if someone wants to set things up so all their 5 or 10 page docs look the
% same, but otherwise the detail is probably too much.
...
\quotation{multi-file projects}.  (This book itself is a multi-file
project, and (at some point in your \ConTeXt\ journey) examining the
structure of its source files may be informative.)
...
table} is used to connect a number with a character.  For text files,
there are several possible tables.  The term {\em text file encoding} refers
to the specific character|-|matching table that a particular text file
uses.
...
\startSmallPrint
  ...
  Western languages, the one that has fewer characters, because it does not
  use any diacritics (accents and other marks above or below or through
  ...
  be represented by a 7-digit binary number, and many early computers used
  the 8-bit byte as a unit for measuring memory.  Any character in the
  table would fit into a single byte.  Since the byte has 8 binary digits
  (bits) and ASCII used only 7 bits, there was even space left to add 128
  other characters, to represent (some) characters from other languages.
  ...
  computer companies gradually tackled the problem on their own.  Therefore,
  not only were specific tables created for different languages or groups
  of languages, but also different tables according to the company that had
  ...
  all the characters that was created, but a standard encoding (called {\sc
  Unicode}) along with different ways of representing it (UTF-8,
  UTF-16, UTF-32, etc.).  Of all these systems, the one that has ended up
  becoming the de facto standard is UTF-8, which makes it possible to
  represent practically any living language, and many already extinct
  languages, as well as numerous additional symbols, all using numbers of
  variable length (between 1 and 4 bytes), which, in turn, helps to
  optimise the size of text files.  This size has not increased {\em too
  much} compared to files using pure ASCII.
...
\dontleavehmode\ConTeXt\ LMTX assumes that the encoding will be UTF-8.
...
processed by \ConTeXt\ LMTX must be saved in UTF-8\footnote{Note that ASCII
    is a subset of UTF-8, so if you have no need for characters other than
    those available in ASCII, your editor may indicate that your file is
    ASCII, not UTF-8; this is perfectly fine}, unless, of course, there is
a very good reason for using a different encoding (although I cannot think
what this reason might be).
...
\item Convert the file to UTF-8, the recommended option; there are various
  tools for doing this; in Linux, for example, the commands {\tt iconv} or
  {\tt recode}.

\item Tell \ConTeXt\ in the source file that the encoding is not UTF-8.  To
  do this we need to use the command \tex{enableregime}, the syntax of
  which is:
...
encoding of the file in question. In \in{Table}[encodings] you will
...
\ConTeXt\ LMTX strongly recommends the use of UTF-8; I agree with this
recommendation.  From here on in this introduction, we assume that the
...
source file. A long with these there is another group of characters that,
...
intents and purposes.  A tab character (which may or may not be entered
into your file by pressing the Tab key on the keyboard, depending on your
editor and its setup) will be transformed into white space by \ConTeXt.
Blank spaces are absorbed into any other blank space (or tab) immediately
following them.  Thus, it makes absolutely no difference in the source file
to write
...
Therefore, if we want to introduce an additional blank space between the
words, we need to use some \ConTeXt{} commands that do this.  Normally it
will work with \quotation{\cmd{\textvisiblespace}}, meaning a
{\tt\backslash} character followed by a blank space. But there are other
procedures that will be looked at in \in{Chapter}[sec:horizontal space1]
regarding horizontal space.
...
knowing that it will not in any way affect the final document.  Thus, in
...
closely enough, you will see a larger amount of space between the first two
words.
...
blank space after \lettertilde\ will not be absorbed either.  (Recall
that \lettertilde\ produces an \quotation{unbreakable space}, for those
situations where we want to insure that two words stay together on the same
line in the output PDF file.)
...
procedures for increasing vertical space.  I refer you to
...
\startSmallPrint
  ...
  character \MyKey{\%} which, as we know, causes the process of the line
  where it appears to be terminated, which implies that the line break at the
  end of the line will also not be processed.  So, for example, the command
...
You might be thinking \quotation{But what about the blanks at the beginning
of the line?  Why do they not produce spaces in the output?}.  The
reason is \ConTeXt\ follows the same rule as \TeX: blanks and tabs at the
beginning of the line are ignored.  This makes it convenient for you to
indent lines when doing so allows you to write your text (or your
macros!\null) in a way that enhances your ability to see the structure of
your \ConTeXt\ source file.

And now you might be thinking \quotation{But how are there lines in this
introduction that start with blanks?}.  Good question!  The answer is that
you can tell \ConTeXt\ to output {\em verbatim} text; that is, the output
is exactly faithful to the input in the \ConTeXt\ source file.  This is
discussed in more detail in \in{Section}[sec:verbatim].  But normally, the
rules for spaces, tabs and line breaks are as described above.
...
printed text.  On a normal keyboard, there is usually only one character
for the dash (or rule, in typographic terms) which we call the hyphen or
...
\item Short rules (hyphens) (-), like those used to separate syllables in
  hyphenation at the end of a line, or for compound words (in English,
  anyway) such as \quotation{thought-provoking}.

\item Medium-sized rules (\quotation{en dashes} or \quotation{en rules})
  (--), slightly longer than hyphens.  They have a number of uses including,
  for some European languages (less so in English) the beginning of a line
  of dialogue, or to separate the lesser from the greater digits in a range
  in dates or pages, such as \quotation{pp. 12--33}.

\item Long rules (\quotation{em dashes} or \quotation{em rules}) (---),
  used as an alternative to parentheses to include one sentence within
  another, to surround a list in the middle of a sentence, and for a number
  of other situations..

\item Minus sign ($-$), to represent subtraction or a negative number.  The
  (typographically) correct way to typeset a negative number in \ConTeXt\
  is to type, for example, \quotation{\type{$-40$}}, \quotation{\type{\im{-40}}}
  or \quotation{\tex{minus40}}, not \quotation{\type{-40}}; compare the
  appearance of $-40$ (any of the first three) to -40.

\stopitemize

Today, all of the above rules, and others besides, are available as single
characters in the UTF-8 encoding.  But since only the hyphen can be
generated by a single key on the keyboard, the others are not so easy to
produce in a source file.  Fortunately, \TeX\ saw the need to include more
rules/dashes in our final document than could be produced by the keyboard,
and designed a simple procedure to do so.  \ConTeXt{} has complemented this
procedure by also adding commands that generate these various kinds of
rules.  We can use two approaches for generating the four kinds of rule:
either the ordinary \ConTeXt{} way with a command, or directly from the
keyboard.  These procedures are shown in
\in{Table}[tbl:rules]:
...
are the names of units of measure used in typography\footnote{We saw
    in \in{Table}[tbl:measurements] that \ConTeXt\ uses \MyKey{em} as a
    unit of measurement, but LMTX does not support \MyKey{en} as a unit.}.
An \quotation{en} is roughly (but not necessarily exactly!\null) the width
of an \quote{n}, while an \quotation{em} is roughly (but not necessarily
exactly) the width of an \quote{m} in the font being used.

% See the comment in b03 about ems and em-dashes and en-dashes.
...
\quotation{multi-file projects}.

The scenarios where it is typical to work with more than one source file
are as follows:

% TODO: The English translation had "festschrift" where I now have
% "commemorative publication".  I'm thinking there has to be a better
% translation of this word, or maybe a better example of a collaborative
% document.  Anyone have a clever suggestion?
...
\startitemize
  ...
  \item If we are writing a document in which a number of authors have
    ...
    example, if we are writing a commemorative publication with
    contributions from different authors, or an issue of a journal, etc.
  ...
  \item If we are writing a lengthy document where each part (chapter) has
    ...
    relative frequency for many academic texts (manuals, introductions and
    the like) where the order of chapters may vary.

  \item If we are writing a number of related documents that share some
  ...
  \item If, put simply, the document we are working on is large, such that
    ...
    considerably speed up the compilation (if we can compile each on
    individually).

  \item Also, if we have written a number of macros that we want to apply
    in some (or all) our documents, or if we have generated a template that
...
essentially appear in the same file.  We differentiate between the
following parts of this file:
...
\startitemize[packed]
  ...
  \item An indication of paper size (\in{Section}[sec:papersize]) and page
    layout (\in{Section}[sec:pagelayout]).

  \item Features of the document's main font (\in{Section}[sec:mainfont]).

  \item Customisation of the section commands to be used
    (\in{Section}[sec:setuphead]) and, if need be, definition of new
    section commands (\in{Section}[sec:definehead]).

  \item Layout of headers and footers (\in{Section}[sec:headerfooter]).

  \item Settings for our own macros (\in{Section}[sec:definingcommands]).
...
processable text, should be there.  In theory, any processable text
included in the preamble will be ignored, although sometimes, if it is
there, it will cause a compiling error.
...
where {\em FileName} is the name of the file to insert.  Note that it is
not necessary for the file name to be enclosed between curly brackets, even
though it will not throw an error if this is done.  However, it should
never be put between square brackets.  If the file extension is
\quotation{\type{.tex}}, it can be omitted.

% NOTE: On March 15 2026 I asked Hans about the lack of an error message
% when the \input file can't be found.  But so far no comment from him
% about that.

% TODO: if one sets TEXMFHOME to something, that replaces ~/texmf as the
% default place for personal files.  This suggests this paragraph (and
% maybe one below which mentions TEXINPUTS) could stand a little more work.

When \ConTeXt\ is compiling a document and finds an \tex{input} command, it
looks for the file indicated and continues compiling as if the contents of
this file were part of the file that called it, at the place
where \tex{input} was found.  When \ConTeXt\ finishes processing
the \tex{input} file, it returns to the original file and continues from
where it left off.  The practical result is, therefore, that the contents
of the file called by means of \tex{input} are inserted at the point where
that is called.  The file called with \tex{input} must have a valid name in
our operating system and no blank spaces within the name.  \ConTeXt\ will
look for it in the working directory, and if it doesn't find it there, it
will look for it in directories specified in the {\tt TEXINPUTS}
environment variable (if you have set that up).  Finally, if you have a
directory called {\tt texmf} in your home directory, \ConTeXt\ will look in
there for \tex{input} files as well.  If the file is not ultimately found,
a compilation error should result\footnote{As of March 2026, an explicit
    error message does not always appear; the {\tt context} run might end
    abruptly for no immediately obvious reason.  If you examine the {\tt
    .log} file or the terminal output, near the very end you might see
    a line like \type{open source     > level 1, order 2, name './FileName.tex'} 
    with no corresponding \type{close source ...} line.  The \type{open}
    with no \type{close} tells you that \ConTeXt\ was unable to find the
    file \type{FileName.tex} and stopped execution.}.
...
% TODO: unless there have been some changes above here which change the
% pagination, there is a great big ugly blank space here.  This should be
% fixed, maybe by turning both of these examples into figures and letting
% them float.  Removing about 5 or 6 lines between them would work as well.
...
\startSmallPrint
    ...
    longer work.  See \in{Section}[sec:references].  Such missing
    references won't cause a compilation error, but you will see things
    like \quotation{section ??} in the part you have compiled for every
    reference to a part you didn't compile.  Don't worry too much about
    these, they will appear when you eventually compile the whole document.
...
% No doubt we could do some \if<something> trickery to get around
% this, but that is probably too sophisticated for a "short" introduction
% like this book.
...
The first argument is similar to that of \tex{ReadFile}: it is the name of
a file enclosed between curly brackets.  The second argument includes the
text to be written if the file exists, before inserting the contents of the
file.  The third argument includes the text to be written if the file in
question is not found.  This means that depending on whether or not the file
entered as the first argument is found, the second argument (if the file
exists) or the third (if the file does not exist) will be executed.

One use (among many) of \tex{readfile} is to put some informative message
in your document if the input file is not found.  For example, you could
use\\
\type{\readfile{Chap23}{}{Chap23 SHOULD HAVE BEEN HERE!  WHERE IS IT?}}
\\
to leave the reader a fairly noticeable message in the final document.
...
The third mechanism that \ConTeXt\ offers for multi-file projects is more
complex and complete: it starts by distinguishing
between \quotation{project} files, \quotation{product}
files, \quotation{component} files and \quotation{environment} files. To
understand the relations and functioning of each of these types of file, I
think it is best to explain them each individually.

\startsubsection
    [reference=environments, title={{\em Environment} files}]
\PlaceMacro{startenvironment}\PlaceMacro{environment}

An {\em environment} file is a file that stores the macros and
configurations of a specific style that is intended to be applied to
several documents, whether they are completely independent documents or
parts of a complex document.  The environment file, therefore, can include
everything we would normally write before \tex{starttext}; that is: the
general configuration of the document.

\startSmallPrint

    I have retained the term \quotation{environment files} for these kinds
    of files, in order not to depart from the \ConTeXt\ official
    terminology, even though I believe that a better term would probably
    be \quotation{format files} or \quotation{global configuration files}.
...
\quotation{\type{env_MyMacros.tex}}.  The inside of such an environment
...
% TODO: the wiki says an environment needs a name.  Test this and modify if necessary.

\startSmallPrint

    With regard to the name of the environment, according to my tests, the
    name we add immediately after \tex{startenvironment} is merely
...
the file taken as an argument.  If that file is not found, \ConTeXt\ will
continue compiling in a normal way without generating any error.  If the
file extension is \quotation{\type{.tex}}, it can be omitted.
...
independently.  Each component is in its own file, and, in addition,
...
various \quotation{component} files and the \quotation{product} file is
limited to establishing the order of the components.  On the other hand,
the (individual) components and the products can be compiled directly.
...
\tex{environment EnvironmentName}.  We can do the same in the product file before
\tex{startproduct}.  Several environment files can be loaded
simultaneously.  We can, for example, have our favourite collection of
macros and the different styles we apply to our documents all in different
files.  Please note, however, that when we use two or more environments,
these are loaded in the order in which they are called, so that if the same
configuration command has been included in more than one environment, and
the command has different values in different environments, the value(s) of
the last loaded environment will apply.  On the other hand, environment
files are loaded only once, so in the previous examples in which the
environment is called from the product file and from specific component
files, if we compile the product, that is the time when the environments
are loaded, and in the order indicated there; when an environment is called
from any of the components, ConTeXt will check if that environment is
already loaded, in which case the environment file will not be processed
again.

The name of the component that is called from a product must be the name of
the file that contains the component in question, although, if the
extension of this file is \quotation{\type{.tex}}, it can be omitted.
...
group a number of products: this is the {\em project}.

A typical project file would look more or less as follows:
...
actually does nothing: it is simply a reminder for the author as to what
products belong to the project.
...
This is why, if we include the 
\PlaceMacro{project}\tex{project ProjectName} command in a component or
product file, \ConTeXt\ will read the project file and automatically load
the environments linked to it.  This is why the \tex{environment} command in
projects has to come after \tex{startproject}; however, in products and
components, \tex{environment} has to come {\em before} \tex{startproduct}
or \tex{startcomponent}
...
\startitemize[a, broad]
  ...
  \item By inserting the \tex{environment EnvironmentName} command before
    \tex{starttext} or \tex{startcomponent}.  This will load the environment
    for compiling the file in question only.

% TODO: is the last sentence of the above item true?

  \item By inserting the \tex{environment EnvironmentName} command in a
    product file before \tex{startproduct}.  This will load the environment
    when the product is compiled, but not if its components are compiled
    individually.
...
and in the path indicated by the {\tt TEXINPUTS} variable.  However, when
we use the \tex{project}, \tex{product}, \tex{component}
or \tex{environment} commands it is assumed that the project has a
directory structure in which common elements are found in the parent
directory, and the specific ones in some child directory.  So, if the file
indicated in the working directory is not found, it will be searched for in
its parent directory, and if it is not found there either, in that
directory's parents directory, and so on.

% TODO: is the above true in LMTX?

...
% TODO: is it not the case that \par and \endgraf end a paragraph?  If so,
...
and produces source files that are easier to read and understand.
Inserting paragraph breaks with an explicit command is something usually
done only inside a macro (see \in{Section}[sec:define]) or in a table cell
(see \in{Section}[sec:tables]).

In a well-typeset document, from a typographical point of view it is
important that the paragraphs stand out visually from each other.  There
are two typical ways of achieving this: by slightly indenting the first
line of each paragraph, or by slightly increasing the blank space between
paragraphs.  Sometimes a combination of both procedures is used, although
in some places this is not recommended because it is considered
typographically redundant.

\startSmallPrint

    I don't totally agree.  The simple indentation of the first line does
    not always visually highlight the separation between paragraphs enough;
    but an increase in spacing not accompanied by indentation poses
    problems in the case of a paragraph that begins on the top of a page:
    we may be unsure whether it is a new paragraph, or a continuation from
...
disabled by default.  We can enable and disable it as desired.  When it is
enabled, the extent of indentation can be indicated with the
\PlaceMacro{setupindenting}\tex{setupindenting} command that allows
...
\startitemize[packed]
  ...
  \item {\tt\bf yes}: enable {\em normal} paragraph indentation.  Certain
    ...
    paragraph of sections, or paragraphs following certain environments,
...
\startSmallPrint
    ...
    indentation was two quads.  In typography, a quad (originally {\em
    quadrat}) was a metal spacer used in letterpress typesetting whose
    height and width are equal to the font size of the typeface being used.
    Thus, with a 12-point font, the quad would measure 12 points wide by 12
    points high.  The term was later adopted as the generic name for two
    common space sizes in typography, regardless of the form of typesetting
    used.  \ConTeXt\ has two quad commands: \tex{quad} that generates one
    space of the kind referred to above, and \tex{qquad} that generates
    twice that amount, but again based on the font being used.  An indent of
    two quads with an 11 point letter will measure 22 points, and with a 12
    point letter, 24 points.
...
\startSmallPrint
    ...
    \tex{setupindenting[yes, big]}.  In this document, however, I haven't
    done this because if indentation were enabled, the large number of
    short sentences and examples would result in a visually untidy
...
\startitemize[packed]
  ...
  \item {\tt\bf after}: command to be run after exiting the environment.
...
where {\tt Name} is the name linked to this configuration and where {\tt
Configuration} allows the same options as \tex{setupnarrower}.
...
\startitemize
  ...
  \item {\tt\bf small, medium, big}: these insert, respectively, a small,
    medium or large vertical space.  The actual size of the space inserted
    by these values depends on the font size.
  ...
  \item {\tt\bf DIMENSION}: establishes a specific dimension for the space
    between paragraphs, independent of the font size.  For
    example, \tex{setupwhitespace[5pt]}.
...
\startitemize
  ...
  \item The symbolic values are so-called elastic dimensions (see
    \in{Section}[sec:dimensions]), meaning that they have {\em normal}
    dimensions but a certain increase or decrease (\quotation{stretch}
    or \quotation{shrink}) in this value is allowed, to assist \ConTeXt\ in
    typesetting pages so that paragraph breaks are aesthetically similar.
    A fixed measure of separation between paragraphs, however, makes it
    more difficult to achieve good pagination for the document.

  \item The symbolic values small, medium, big, etc., are calculated on the
    basis of font size, so if font size changes in certain parts, it will
    also change the amount of vertical spacing between paragraphs, and the
    end result will always be harmonious.  Conversely, a fixed value for
    vertical spacing will not be affected by changes in font size, which
    will normally translate into a document with poorly distributed white
    space (from the aesthetic point of view) and not in accordance with the
    rules of typographical adjustment.

\stopitemize

% TODO: is it worth pointing out that someone could use a "specific
% dimension" like "2ex plus 0.5 ex minus 0.5 ex", which would allow a
% specific measure, but one that varies with the font size?
...
these commands are rarely needed, because \ConTeXt\ manages the vertical
spacing between paragraphs quite well on its own; especially if one of the
predefined dimensions, calculated from the current active font size and
spacing, has been entered as the value.
...
\startSmallPrint
    ...
    spacing for particular paragraphs given that vertical spacing has
    already been generally established \Conjecture for all paragraphs?
    However, when writing advanced macros, \tex{whitespace} can be useful
    in the context of a loop that has to make a decision based on the value
    of a certain condition.  This is more or less advanced programming, and
...
where {\tt Space} is an optional argument indicating what amount of
vertical space is desired between the paragraphs in the environment.  If
{\tt Space} is omitted, no extra vertical space will be applied.

\stopsubsection


\startsubsection
    [title={Adding additional vertical space at a particular point in the
            document}]

If, at a particular point in the document the normal vertical spacing
between paragraphs is not sufficient, we can use
the \PlaceMacro{blank}\tex{blank} command.  Used without arguments,
both \tex{blank} and \tex{blank[big]} will insert the same amount of
vertical space as has been set with \tex{setupwhitespace}.  But we can
indicate either a specific dimension between square brackets, or one of the
symbolic values calculated from the font size: small, medium or big.  We
can also multiply those sizes by some whole number, and so on, for
example, \tex{blank[3*medium]} will insert the equivalent of three medium
line breaks.  We can also put two sizes together.  For
example, \tex{blank[2*big, medium]} will insert two large and a medium
break.
...
typographical practice), in the configuration of \tex{chapter} command we
should write something similar to the following:
...
using the \MyKey{force} option, no extra space will appear.
...
\tex{blank[big]}.  However, we can change the amount of space
that \tex{blank} (with no arguments) outputs with \tex{setupblank}, setting
it as \tex{setupblank[0.5cm]} for example, or \tex{setupblank[medium]};
this does not affect the amount of space output by \tex{blank[big]} (or
\type{medium} or \type{small}, either).  Used without
arguments, \tex{setupblank} will adjust the value to the size of the
current font.

Just as with \tex{setupwhitespace}, the white space inserted
by \tex{blank}, when its value is one of the predefined symbolic values, is
an elastic dimension that allows for some adjustment.  We can change this
with \MyKey{fixed}, with the possibility, later on, of restoring the
default value with (\MyKey{flexible}). Thus, for example, for text in
double columns it is recommended to set \tex{setupblank[fixed, line]}, and
when going back to a single column \tex{setupblank[flexible, default]}.
...
macros.  In its place it is suggested to
use \PlaceMacro{godown}\tex{godown} whose syntax is:

\type{\godown[Dimension]}

where {\em Dimension} needs to be a number with or without decimals,
followed by a unit of measure.  For example, \tex{godown[5cm]} will
instruct \ConTeXt\ to add a gap of 5~cm.
If there is less than 5~cm of space left on the current page,
\tex{godown} will cause a page break (and the \quotation{left-over} space
will not appear at the top of the next page). Similarly,
...
that we are using the \quote{\textvisiblespace} character in this
document to represent a blank space when it is important for us to see
it.} that will first insert a blank space that will mean we are no
longer at the beginning of the page, and will then go down three
centimetres.  (Caution: this will move down a bit more than 3~cm, because
there is now a line (with no text) at the top of the page.  This can be
avoided, but the explanation of how to do so is a bit sophisticated for
this introduction.)
...
\startSmallPrint
    ...
    \tex{godown[3cm]} is practically the same.  However, there are some
    subtle differences between them.  For example, two
    consecutive \tex{blank} commands will not be accumulated; when this
    happens, only the one that imposes a greater distance is applied.  Two
    or more \tex{godown} commands, on the other hand, can accumulate
    perfectly.

    At first the \quotation{non-accumulating} behaviour of \tex{blank}
    might seem strange.  But consider a command which is designed to leave
    a bit of space {\em after} whatever output it creates, and a second
    command which is designed to put a bit of space {\em before} its
    output.  If in some document the second command immediately follows the
    first, we (normally) wouldn't want two have the two space amounts
    added: \tex{blank} takes care to only use the larger of the two spacing
    amounts, not the sum of the two amounts.

\stopSmallPrint

Another rather useful \TeX\ command, the use of which poses no problems in
\ConTeXt, is \PlaceMacro{vfill}\tex{vfill}.  This command inserts a
flexible vertical blank space going as far as the bottom of the page.  It is
as if the command {\em pushes} down what is written after it.  This allows
for interesting effects such as how to place a certain paragraph at the
bottom of the page, by simply preceding it with \tex{vfill}. Now, the
effect of \tex{vfill} is difficult to appreciate if its use is not combined
with forced page breaks, because there is little point in pushing a
paragraph or line of text down if the paragraph, as it grows, grows
upwards.
...
In the first \tex{vfill} example, the space is there in case someone tries
to create a sample \ConTeXt\ file with only the bottom three lines.  But
one could equally have a paragraph at the top of the page, followed by
the \tex{vfill}, the \quotation{Line at the bottom}, and the \tex{page[yes]}.
...
its entirety, so that even the {\em final} words of a paragraph can
influence the division of the {\em first} line.  As a result, the space
between the words in the entire paragraph is as uniform as possible.
...
\startSmallPrint
    ...
    \ConTeXt.  Because a word processor, when it reaches the end of the
    line and moves to the next, adjusts the white space in the line just
    finished to fill out the line.  It does this with each line, and at the
    end, each line in the paragraph will usually have different interword
    spacing.  This can cause a very bad effect (e.g., a line with large
    interword spaces next to a line with small interword spaces can be
    visually jarring).  \ConTeXt, on the other hand, processes the
    paragraph in its entirety, and for each line calculates how many
    breakpoints are admissible and the amount of interword spacing that
    would result from each such line break.  As the breakpoint of a line
    affects the potential breakpoints of the next lines, the total number
    of possibilities can be very high; but that is not a problem
    for \ConTeXt.  It will make a final decision based on the entire
    paragraph, ensuring that the space between words on each line is {\em
    as similar as possible}, which results in paragraphs which are visually
    more appealing and less distracting than those from cruder systems.

\stopSmallPrint

To do this, \ConTeXt\ tests different alternatives, and assigns a so-called
\quotation{badness} value to each alternative, based on its internal
parameters.  (These parameters were established after an in-depth study of
the art of typography.)  Finally, after having explored all
possibilities, \ConTeXt\ chooses the least unsuitable option (the one with
the least badness value).  In general, this functions quite well, but there
will inevitably be cases where line breakpoints are chosen that are not the
best, or that do not appear to us to be the best.  Therefore, sometimes we
will want to tell the program that certain places are not good break
points.  Conversely, on other occasions we will want to force a break at a
particular point.  The following subsections investigate some of the ways
we might influence line breaking.
...
The main candidates for line breakpoints are obviously the spaces between
words.  To indicate that a certain space should never be replaced by a line
break, we use, as we already know, the \quote{\lettertilde} reserved
character, which \TeX\ calls a {\em tie}, tying two words together.  (Some
systems use the expression \quotation{non-breakable space}.)
...
% TODO: do we put spaces in the middle of abbreviations in English?
%   If someone says yes, we can uncomment this.
...
%      U\lettertilde S}.
  ...
  \item Between digits and the symbols preceding or following them so long
    as they are not superscripts. For example, {\tt 73\lettertilde km},
    {\tt 53\lettertilde €}; however, not in cases like {\tt 35'}.
  ...
  \item In groups of numbers separated by white space. For example, {\tt
    ...
    preferable to use what is called a {\em thin space}, achieved in \ConTeXt\
...
\startitemize[packed]
  ...
  \item When mathematical expressions are expressed in words. For example, {\tt
  equal\lettertilde to\lettertilde \$n\$}.
...
break between two syllables of a word; to do this it is essential for
\ConTeXt\ to know the language the text is in, since hyphenation rules are
different for each language. Thus the importance of the \tex{mainlanguage}
command in the document preamble.

It can happen that \ConTeXt\ is unable to hyphenate a word suitably.
...
if either part does not have a particular minimum number of letters), other
times because the word is ambiguous.  After all, what might \ConTeXt\ do
with the word \quotation{unionised}?  The word could appear in a phrase
like \quotation{the unionised workforce}, but it could also appear in a
chemistry text as \quotation{an unionised atom} (i.e., not ionised).
And what if \ConTeXt\ had to deal with the word \quotation{manslaughter} as
the last word on a page before a page break.  It may split the word as
man-slaughter (correct) but it may also split it as mans-laughter
(ambiguous or just wrong).
...
creates an invisible but indivisible horizontal box around the word, or
...
\startSmallPrint
    ...
    Internally, how hyphenation works is partially controlled by the \TeX\
    ...
    \ConTeXt\ always considers acceptable any line break that does require
    hyphenation, meaning that {\em de facto}, we are removing hyphenation
    based on syllables.  Conversely, if we set the \tex{pretolerance} value
    to $-1$, we would be forcing \ConTeXt\ to consider using hyphenation at
    the end of the line every time.
    ...
    and \MyKey{morehyphenation} values in \tex{setupalign}; see 
    \in{Section}[sec:setupalign] for details.

    % TODO: JAL had no explanation of \tolerance.  Is this OK (for an
    % introductory level document like this)?

    While \tex{pretolerance} gives control whether \ConTeXt\ will try to
    hyphenate words on its first try to break the paragraph into
    lines, \tex{tolerance} controls how fussy \ConTeXt\ will be for the
    finished paragraph.  Setting \tex{tolerance=10000} tells \ConTeXt\ that
    it can output very \quotation{sloppy} paragraphs; normally this would
    be undesired.
...
provides optimal results, and only exceptionally do certain lines stick out
...
and then makes an appropriate decision about how to deal with the problem.
This could mean inserting a \tex{break} command before the word that
extends beyond, or could also mean wording the paragraph differently so
that this word shifts position elsewhere.  A slight rewording is often
sufficient to fix problems like this.

However, in some cases, \ConTeXt's low tolerance can be a problem.  In
these cases we can tell it to be more tolerant with white space in lines.
We have the \PlaceMacro{setuptolerance}\tex{setuptolerance} command for
this, allowing us to alter the level of tolerance in calculating line
breaks, which \ConTeXt\ calls \quotation{horizontal tolerance} (because it
affects horizontal space) and \quotation{vertical tolerance} when
calculating page breaks.  We will talk about this
in \in{Section}[sec:VerticalAlignment].
...
words on that line.  This can be seen in the following example, in which the
...
To avoid this effect, we can use the \cmd{\ttbs} or \tex{crlf}
...
\cmd{\ttbs} and \tex{crlf}; but in a section title there is a
difference:

\startitemize

  \item {\bf \tex{\}} generates a line break in the body of the document, but not
      when the section title is transferred to the table of contents.
...
\startitemize
  ...
  \item In very exceptional cases when \ConTeXt\ has not been able to find
    ...
    {\em boxes}, or {\em verbatim} text [see \in{Section}[sec:verbatim]]),
    it could be helpful to force a line break with \tex{break} just before
    the word that protrudes into the right margin.
  ...
  \item In paragraphs that are actually made up of individual lines, each
    ...
    line has the author's name, another their office or academic position
    and perhaps a third line with the date, etc.  In these cases the line
    break should be forced with the \tex{\} or \tex{crlf} commands.  It is
    also common for these kinds of paragraphs to be right-aligned.

  \item When writing poems or similar kinds of texts, to separate one line
    of a verse from another.  Although in this latter case it is preferable
    to use the {\tt lines} environment explained
    in \in{Section}[sec:startlines].
...
actual font being used, and, above all, on its base size, set with
...
\startitemize
  ...
  \item \tex{setupinterlinespace [..Interline space..]}, where {\em
    ...
    \startitemize
      ...
      % TODO: Is this accurate, or should the end be "default point size" ?

      \item When it is a specific value it can be a dimension (for example,
        15pt), or a simple whole or decimal number (for example, 1.2).  In
        this latter case the number is interpreted as \quotation{number of
        lines} based on \ConTeXt's default line spacing.  A problem with
        setting the interline space to a size which isn't related to the
        font size (e.g., 15pt) is that if the document's font size is
        changed, 15pt might not be suitable.  If a size with units is
        really desired (the first case), something like (for example) 3ex
        is usually better, since \quotation{ex} is a function of the
        current font size.

      \item When it is a symbolic word this can be \MyKey{small},
        \MyKey{medium} or \MyKey{big}, each of which applies a small,
        medium or big interline space respectively, always based on the
        default interline space \ConTeXt\ would apply.

    \stopitemize

  \item \tex{setupinterlinespace [...=...,...]}.  In this mode, the
    interline space is set by explicitly altering the measurements with
    which \ConTeXt\ calculates the appropriate interline spacing.  I have
    previously said that line spacing is calculated on the basis of the
    specific font and its size; but that was to keep things very simple: in
    reality, the font and its size establish certain measures, from which
    the interline space is calculated. By means of
    this \tex{setupinterlinespace} approach, these measures are modified
    and therefore, so is the interline space.  The actual measures and
    values that can be manipulated by this procedure (the meaning of which
    I will not explain because it goes beyond the scope of a simple {\em
        introduction}), are: {\tt line}, {\tt height}, {\tt depth}, {\tt
        minheight}, {\tt mindepth}, {\tt distance}, {\tt top,} {\tt
        bottom}, {\tt stretch} and {\tt shrink}.

  \item \tex{setupinterlinespace [Name]}.  With this command, we establish
    or configure a specific and customised type of line spacing previously
    defined
    with \PlaceMacro{defineinterlinespace}\tex{defineinterlinespace}.
...
would write \tex{setupinterlinespace[reset]}.
...
\startsubsection
  [
    ...
    title={Converting line breaks in the source file into line breaks in
        the final document},
  ]
...
blank spaces, unless there are two or more consecutive line breaks (i.e.,
one or more blank lines), in which case a paragraph break will be inserted.
However, there may be some situations in which we are interested in
respecting the line breaks of the original source file as they were put
there, for example, when writing poetry.  For this, \ConTeXt\ offers us
the \MyKey{lines} environment whose format is:
...
\startitemize
  ...
  \item {\tt\bf space}: when this option is set with the \MyKey{on} value,
    ...
    temporarily ignoring the \quotation{absorption} rule.

  \item {\tt\bf before}: text to insert or command to run before entering the
    environment.

  \item {\tt\bf after}: text to insert or command to run after exiting the
    environment.

  \item {\tt\bf inbetween}: text to insert or command to run
    between \quotation{paragraphs}.
...
explained generally for this environment.  Once we have defined our
...
numbering.  For example, in texts on computer programming it is
relatively common for the code fragments offered as examples to have their
lines numbered; poems and critical editions are two other examples, but
there are many more.  For all of these situations \ConTeXt\ offers the {\tt
linenumbering} environment whose format is
...
\startitemize
  ...
  \item {\tt\bf continue}: in cases where there is more than one fragment
    of our document whose lines we want to number, this option controls
    whether or not the numbering restarts for each fragment.  With
    \MyKey{continue=no} (the default value), the line numbers are reset
    for this fragment.  But if line numbering is meant to continue on from
    where the previous fragment left off, we choose \MyKey{continue=yes}.

  \item {\tt\bf start}: indicates the number of the first line in cases
    where we do not want it to be \quote{1}, nor for it to continue on from
    the previous fragment.

  \item {\tt\bf step}: this option causes the line number to be printed on
    at certain intervals (although all lines included in the environment
    will be counted).  With poems, for example, it is common that the
    line number only appears in multiples of 5 (lines 5, 10, 15, \unknown).
...
\startitemize
  ...
  \item {\tt\bf conversion}: line numbering type.  It can be any of the
    ones explained on \at{page}[Num:conversion] regarding the numbering of
    chapters and sections.

  \item {\tt\bf style}: command (or commands) determining the style the
    line numbering will have (font, size, variant, \unknown).
  ...
  \item {\tt\bf location}: where the line number will be placed.  It can be
    any of the following: {\tt text}, {\tt begin}, {\tt end}, {\tt
        default}, {\tt left}, {\tt right}, {\tt inner}, {\tt outer}, {\tt
        inleft}, {\tt inright}, {\tt margin}, or {\tt inmargin}.

  \item {\tt\bf distance}: distance between the line number and the line
    itself.

  \item {\tt\bf align}: number alignment.  Can be: {\tt inner}, {\tt
      outer}, {\tt flushleft}, {\tt flushright}, {\tt left}, {\tt right},
    {\tt middle}, or {\tt auto}.
  ...
  % TODO: explain these.  left and right are commands, but margin is a dimen
...
\PlaceMacro{definelinenumbering}\tex{definelinenumbering}, so that the
configuration is associated with a name:
...
\PlaceMacro{setupalign}\tex{setupalign}.  This command is used to control
...
\startitemize[n]
  ...
  \item Accumulate it on one of the two sides of the line: if we accumulate
    it on the left hand side, the line will look {\em a little pushed} to
    the right, while if we accumulate it on the right hand side the line
    remains on the left hand side.  We are talking, in the former case,
    about {\em right alignment} and, in the latter, about {\em left
    alignment}.  By default, \ConTeXt\ applies left alignment to the
    last line of paragraphs.

    % The following paragraph replaces the commented one immediately below,
    % which I thought was a bit long and maybe had some details that the
    % introductory reader doesn't need to know.

    When several consecutive lines are left-aligned, the right hand side is
    irregular; but when the alignment is on the right, the side that looks
    uneven is the left.  To achieve left-aligned text, the {\tt flushleft}
    option is used; analogously, for right-aligned text the {\tt
        flushright} option is used.\footnote{A historical curiosity
        of \ConTeXt\ is that there are also {\tt left} and {\tt right}
        options which do the opposite of what you might think.  That is,
        rather than {\tt left} performing left-alignment, it causes the
        text to be set right-aligned, because another name for
        right-aligned text is \quotation{ragged-left}.  To avoid confusion,
        it is recommended to use neither {\tt left} nor {\tt right} as
        options for \tex{setupalign}.}

    % To name the options that align one or other side, \ConTeXt\ does not
    % set the side where they are aligned, but the side where they are
    % uneven.  Therefore, the {\tt flushright} option results in left
    % alignment and {\tt flushleft} in right alignment. As abbreviations of
    % {\tt flushright} and {\tt flushleft}, \tex{setupalign} also supports
    % {\tt right} and {\tt left} as values. But {\bf attention}: here the
    % meaning of the words is deceptive. Even though {\em left}
    % means \quotation{left} and {\em right} means \quotation{right},
    % \tex{setupalign[left]} aligns on the right and \tex{setupalign[right]}
    % aligns on the left. In case the reader wonders why this comment has
    % been made, it would be worth quoting from the \ConTeXt\ wiki:
    % \quotation{ConTeXt uses flushleft and flushright options. The right and
    % left alignments are backwards from the usual directions in all
    % commands that accept an alignment option, in the sense
    % of \quote{ragged left} and \quote{ragged right}.  Unfortunately, when
    % Hans was first writing this part of ConTeXt, he was thinking
    % of \quote{ragged right} and \quote{ragged left} alignment, rather
    % than \quote{flush left} and \quote{flush right}.  And now that it's been
    % this way a while, it's impossible to change it, because changing it
    % would break backward compatibility with all of the existing documents
    % that use it.}

    In documents prepared for double-sided printing, in addition to
    referring to right and left margins, one can also refer to inner and
    outer margins (the inner margins are those between the pages).  The
    values {\tt flushinner} (or simply {\tt inner}) and {\tt flushouter}
    (or simply {\tt outer}) establish the corresponding alignment in these
    cases.

  \item Distribute the extra space across both margins.  The result will be
    that the line is centred.  The \tex{setupalign} option that does this
    is {\tt middle}.
  ...
  \item Distribute it among all the words making up the line, if necessary
    ...
    same width as the space available to it.  In these cases we talk about
    {\em justified lines}.  This is also \ConTeXt's default value which is
    why there is no special option in \tex{setupalign} to establish it.
    However, if we have altered alignment justified by default, we can
    restore it with \tex{setupalign[reset]}.

\stopitemize

The values for \tex{setupalign} that we have just seen ({\tt right}, {\tt
    flushright}, {\tt left}, {\tt flushleft}, {\tt inner}, {\tt
    flushinner}, {\tt outer}, {\tt flushouter} and {\tt middle}) can be
combined with {\tt broad}, which results in somewhat rougher alignment.

\startSmallPrint

    Two other possible values for \tex{setupalign} that affect horizontal
    alignment have to do with the hyphenation of words at the end of the
    line.  Whether hyphenation occurs or not affects the amount of white
    space.

    Specifically, \tex{setupalign} accepts the {\tt morehyphenation}
    option, which makes \ConTeXt\ work harder to find breakpoints based on
    hyphenation, and {\tt lesshyphenation} which produces the opposite
    effect.  With \tex{setupalign[horizontal, morehyphenation]}, the
    remaining white space in the lines will (generally) be reduced and
    therefore the extra white space to achieve alignment will be less
    apparent.  On the contrary, with \tex{setupalign[horizontal,
    lesshyphenation]}, there will be more left-over white space, and the
...
\startitemize
  ...
  \item The \MyKey{alignment} environment, intended to affect several
    ...
    where {\tt Options} are any of those allowable for \tex{setupalign}.
  ...
  \item The \PlaceMacro{leftaligned}\tex{leftaligned},
    ...
    centred or right alignment respectively; and if we want the last word
    in a paragraph (but only this and not the rest of the line) to be right
    ...
    \startSmallPrint
        ...
        \MyKey{left} in \tex{setupalign} cause the opposite alignment to
        what the name suggests, the same does not happen with
        the \tex{leftaligned} and \tex{rightaligned} commands that bring
        about exactly the kind of alignment that their name suggests: {\tt
        left} on the left, and {\tt right} on the right.
...
While horizontal alignment comes into play when the width of a line does
not take up all the space available to it, vertical alignment affects the
height of the whole page.  If the {\em exact} text height of a page does
not take up all the height available to it, what do we do with the
remaining white space?  We can pile it up at the top (\MyKey{height}),
which means that the text on the page will be pushed down; we can pile it
up at the bottom (\MyKey{bottom}) or distribute it among the paragraphs
(\MyKey{line}).  The default value for vertical alignment
is \MyKey{bottom}.
...
vertical tolerance, i.e., tolerance for space between paragraphs larger
than what \ConTeXt, by default, considers reasonable for a well-typeset
page.  The possible options for vertical tolerance are the same as for
horizontal tolerance: {\tt verystrict}, {\tt strict}, {\tt tolerant} and
{\tt verytolerant}.  The default value is 
...
widows and orphans.  Both phenomena refer to the situation in which a page
break causes one line of a paragraph to be isolated on a different page
from the rest of the paragraph.  This is not considered to be
typographically desirable.  If the line that is separated from the rest of
the paragraph is the first one on the page, we are talking about a {\em
widowed line}; if the line separated from its paragraph is the last one
on the page then we are talking about an {\em orphaned line} (orphans are
called \quotation{clubs} by some people).
...
alternative is even worse from a typographical point of view.
